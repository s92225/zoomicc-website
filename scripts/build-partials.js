...
const baseFooter = [
  ["Pages", [["About", "index.html#about"], ["Services", "index.html#capabilities"], ["Resources", "automation-blueprints.html"], ["Pricing", "pricing.html"], ["Signals", "index.html#lab"]]],
...
function markCurrent(nav, label) {
  return nav.map((item) => item[0] === label ? [item[0], item[1], item[2] || "", "page"] : item);
}
...
function renderNavLinks(items) {
  return items.map(([label, href, className, current]) => {
    const attrs = [
      `href="${href}"`,
